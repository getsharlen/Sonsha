                    ...
                    <div class="rounded-2xl bg-slate-950/60 p-4">
                        <div class="flex items-center justify-between gap-3">
                            ...
                            <div class="flex gap-2">
                                <form method="POST" action="/borrowings/{{ $borrowing->id }}/approve" class="inline confirm-action" data-confirm="Setujui peminjaman ini?">
                                    @csrf
                                    <button class="rounded-xl bg-emerald-500 px-3 py-2 text-xs font-semibold text-white hover:bg-emerald-600">Approve</button>
                                </form>
                                <form method="POST" action="/borrowings/{{ $borrowing->id }}/decline" class="inline confirm-action" data-confirm="Tolak peminjaman ini?">
                                    @csrf
...
<div class="mt-6 grid gap-6">
    <section class="glass-panel rounded-3xl p-5">
        <div class="flex items-center justify-between">
            <h3 class="text-lg font-semibold">Log Laporan Terbaru</h3>
            <a href="/reports/technical" class="rounded-xl border border-pink-500/30 bg-pink-500/10 px-3 py-2 text-xs font-semibold text-pink-300 hover:bg-pink-500/20">
                <i class="fas fa-external-link-alt mr-1"></i> Lihat Semua
            </a>
        </div>
        <div class="mt-4 max-h-80 space-y-3 overflow-y-auto text-sm">
            @forelse($recentActivityLogs as $log)
                <div class="rounded-2xl border border-white/5 bg-slate-950/60 p-4">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="text-xs text-slate-400">{{ optional($log->created_at)->format('d M H:i') }}</span>
                    </div>
                    <p class="mt-1 text-slate-300">{{ $log->description }}</p>
                    <p class="text-xs text-slate-400">Oleh: {{ $log->user?->name }}</p>
                </div>
            @empty
                <p class="text-slate-300">Belum ada aktivitas.</p>
            @endforelse
        </div>
    </section>
</div>
